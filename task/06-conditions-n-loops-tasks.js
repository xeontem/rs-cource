...
function isInsideCircle(circle, point) {
    return (circle.radius > Math.hypot(Math.abs(point.x - circle.center.x), Math.abs(point.y - circle.center.y)));
}
...
function isBracketsBalanced(str) {
  ...
  let arr = str.split('');
  let sw = true;
  if (!(~obraces.indexOf(arr[0]))) return false;
    while(sw){
        for (let i = 0; i < arr.length - 1; i++) {
            let oind = obraces.indexOf(arr[i]);
            let cind = cbraces.indexOf(arr[i + 1]);
            if (~oind && cind == oind) {
                arr.splice(i, 2);
                sw = true;
                if(!arr.length) sw = false;
                break;
            } else sw = false;
        }
    }    
...
function timespanToHumanString(startDate, endDate) {
    ...
    if (date >= 0 && date <= (45 * sec)) return `a few seconds ago`;
    if (date > (45 * sec) && date <= (90 * sec)) return `a minute ago`;
    if (date > (90 * sec) && date <= (45 * min)) return `${Math.round((date - 1) / min)} minutes ago`;
    if (date > (45 * min) && date <= (90 * min)) return `an hour ago`;
    if (date > (90 * min) && date <= (22 * hour)) return `${Math.round((date - 1) / hour)} hours ago`;
    if (date > (22 * hour) && date <= (36 * hour)) return `a day ago`;
    if (date > (36 * hour) && date <= (25 * day)) return `${Math.round((date - 1) / day)} days ago`;
    if (date > (25 * day) && date <= (45 * day)) return `a month ago`;
    if (date > (45 * day) && date <= (345 * day)) return `${Math.round((date - 1) / month)} months ago`;
    if (date > (345 * day) && date <= (545 * day)) return `a year ago`;
    if (date > (545 * day)) return `${Math.round((date - 1) / year)} years ago`;
}
...
function getCommonDirectoryPath(pathes) {
    ...
    let res = [];
    let sw = true;
    function divide(element, i, a) {
        arr.push(element.split('/'));
    }
    pathes.forEach(divide);
    for(let i = 0; i < arr[0].length; i++){
        for(let j = 1; j < arr.length; j++){
            if(arr[0][i] != arr[j][i]) sw = false;          
        }
        if(sw)res.push(arr[0][i]);
    }
    res[res.length-1] += `/`;
...
function getMatrixProduct(m1, m2) {
    let res = [];
    for (let i = 0; i < m1.length; i++) {//m1 rows
        let row = [];
        for (let j = 0; j < m2[0].length; j++) {//m2.row item
            let prod = 0;
            for (let k = 0; k < m2.length; k++){//m2 rows
                prod += (m1[i][k] * m2[k][j]);// product m1 row by m2 column
            }  
            row.push(prod);
        }
        res.push(row)
