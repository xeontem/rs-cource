...
function getPowerFunction(exponent) {
    // return Math.pow.bind(null, exponent);
...
function getPolynom(...args) {
        ...
        return function(z){
            arr = [];
            if(!z) return args[args.length-1];
          for(let i = 0; i < args.length; i++){
            arr.push((args[i]) * (Math.pow(z, args.length - i - 1)));
          }
          arr[arr.length-1] = args[args.length-1];
          return arr.reduce((y,x) => y += x);
        }
}
...
function logger(func, logFunc) {
    return function(...param){
        ...
        for(let i = 0; i < param.length; i++){
            strArgs.push(JSON.stringify(param[i]));
        }
...
function partialUsingArguments(fn, ...args) {
    let str = '';
    if(args) str = args.reduce((y,x) => y+=x);
    return function(...args){
        if(args[0]) str += args.reduce((y,x) => y+=x);
        return str;
    }
